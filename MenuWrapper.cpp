...
namespace SOUI
{
	...
	void MenuItemWrapper::SetIcon(int nIcon)
	{
		AppendItemAttr(L"icon").set_value(nIcon);
	}
	...
	bool MenuItemWrapper::GetCheck()
	{
		return AppendItemAttr(L"check").as_bool(-1);
	}
	...
	void MenuWrapper::AddSeperator()
	{
		if (!_menuRoot)
			return;

		pugi::xml_node node = _menuRoot.append_child(L"sep");
		MenuItemWrapper menuItem(node);
		_menuItems.push_back(menuItem);
		VecMenuItems::iterator it = _menuItems.end() - 1;
	}
	...
	void MenuWrapper::AddChildMenu(const SStringW& str,
		int id,
		BOOL bEnable, /*=TRUE*/
		BOOL bCheck)  /*=FALSE*/
	{
		if (!_menuRoot)
			return;
		pugi::xml_node parentNode = _menuRoot.last_child();
		if (!parentNode)
			return;
		pugi::xml_node node = parentNode.append_child(L"menuItem");
		...
		//MenuItemWrapper menuParentItem(parentNode);
		//int nCount = map_menuChildItems.count(menuParentItem);
		//if (0 == nCount)
		//{
		//	VecMenuItems menuChildItems;
		//	menuChildItems.push_back(menuItem);
		//	map_menuChildItems.insert(std::make_pair(menuParentItem, menuChildItems));
		//}
		//else
		//{
		//	map_menuChildItems[menuParentItem].push_back(menuItem);
		//}
		
		/*std::map<MenuItemWrapper, VecMenuItems>::iterator it = map_menuChildItems.end() - 1;
